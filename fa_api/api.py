...
class FaAPI:
    ...
    def __request(self, sub_url):
        ...
        if r.status_code == 200:
            return r.json()
        raise requests.exceptions.BaseHTTPError("[Ошибка] RUZ отдал код {}!\nURL: '{}'".format(r.status_code, self.base_url+sub_url))
    ...
    def timetable_auditorium(self, auditorium_id: str, date_begin=None, date_end=None) -> list:
        ...
        r = self.__request("/api/schedule/auditorium/{}?start={}&finish={}&lng=1".format(auditorium_id, date_begin, date_end))
        return r
